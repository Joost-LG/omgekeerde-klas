...
function renderTokenPanel() {
  ...
  if (!data.tokens.length) {
    c.innerHTML = '<p class="tekst-grijs">Nog geen tokens aangemaakt.</p>';
    document.getElementById('token-acties').style.display = 'none';
    return;
  }
...
function kopieerToken(t) {
  ...
  chips.forEach(c => { if(c.textContent.trim()===t) { c.style.background='var(--geel-licht)'; setTimeout(()=>c.style.background='',600); } });
}
...
// ══════════════════════════════════════════════════
// QR CODE — zelfstandige implementatie (geen externe library)
// ══════════════════════════════════════════════════
const QRGen = (() => {
...
function toonQR() {
  ...
  document.getElementById('qr-svg-container').innerHTML = QRGen.toSVG(link, 200);
...
function renderAlleVragen() {
  ...
  huidigOpdracht.vragen.forEach((v,i) => {
    ...
    let html = `<div class="vraag-meta">Vraag ${i+1} van ${huidigOpdracht.vragen.length}</div>`;
    html += `<div class="vraag-tekst">${v.tekst || '(geen tekst)'}</div>`;

    if (v.type === 'mc') {
      html += (v.opties||[]).map((opt,j) => opt ?
        `<button class="optie-knop" id="optie-${i}-${j}" onclick="kiesOptie(${i},${j})"><span style="font-family:'DM Mono',monospace;font-size:0.78rem;font-weight:700;margin-right:0.6rem;color:var(--grijs-donker);">${'ABCD'[j]}</span>${opt}</button>` : '').join('');
    } else {
      html += `<div class="open-antwoord">
        <label>Jouw antwoord</label>
        <textarea class="invoer antwoord" rows="4" placeholder="Schrijf hier je antwoord…" oninput="updateOpenAntwoord(${i},this.value)">${studentAntwoorden[i]||''}</textarea>
      </div>`;
    }
    div.innerHTML = html;
    c.appendChild(div);
  ...
  sectie.className = 'inzend-sectie';
  sectie.innerHTML = `
    <p class="inzend-hint">Controleer je antwoorden voordat je inzendt. Je token is daarna niet meer geldig.</p>
    <button class="knop primair" id="inzend-knop" onclick="inzenden()">Antwoorden inzenden →</button>`;
  c.appendChild(sectie);
}
...
// ══════════════════════════════════════════════════
// URL-PARAMETER: direct naar student login als opdracht meegegeven
// ══════════════════════════════════════════════════
(function() {
  const params = new URLSearchParams(location.search);
